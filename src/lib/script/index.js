...
Respond with ONLY the narration text itself, nothing else.`;
  ...
  let script = await generateText({
    prompt,
    temperature: 1,
    // فیکسِ ۲۰۲۶-۰۸-۲۷ — قبلاً اینجا فرض شده بود reasoning_effort="low"
    // واقعاً به Groq فرستاده می‌شه، ولی lib/providers/registry.js هیچ‌وقت
    // این پارامتر رو تو بدنه‌ی درخواست نمی‌ذاشت؛ یعنی مدل عملاً با
    // پیش‌فرضِ خودِ Groq («medium»، نه «low») اجرا می‌شد که توکنِ پنهانِ
    // بیشتری صرف می‌کنه. با سقفِ قبلیِ ۷۰۰، این باعث می‌شد گاهی کلِ
    // سهمیه صرفِ reasoning بشه و content کاملاً خالی برگرده («پاسخ خالی
    // از هوش مصنوعی دریافت شد»). حالا که registry.js واقعاً
    // reasoning_effort:"low" می‌فرسته این کمتر پیش میاد، ولی برای حاشیه‌ی
    // امنِ بیشتر (چون میزانِ توکنِ reasoning حتی با "low" کمی نوسان
    // داره) سقف رو هم به ۱۲۰۰ بردیم بالا — برای یک اسکریپتِ ۹۰-۱۳۰
    // کلمه‌ای (~۱۵۰-۲۰۰ توکنِ خروجی) این حاشیه‌ی امنِ خیلی راحت‌تری می‌ذاره.
    // اگه بازم خالی برگرده، router.js حالا همون provider رو چندبار
    // دوباره امتحان می‌کنه و بعد می‌ره سراغِ provider بعدی، به‌جای این‌که
    // کلِ تولیدِ اسکریپت رو بترکونه.
    maxTokens: isShort ? 1200 : 3000,
  });
