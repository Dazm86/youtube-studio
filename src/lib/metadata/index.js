...
function heuristicMetadata(script) {
  ...
  return {
    ...
    thumbnailTextA,
    thumbnailTextB,
    mayaExpressionA: "warm, confident smile",
    mayaExpressionB: "curious, head tilted",
...
{"titleA": "...", "titleB": "...", "thumbnailTextA": "...", "thumbnailTextB": "...", "mayaExpressionA": "...", "mayaExpressionB": "...", "description": "...", "tags": ["...", "..."]}

Rules:
- titleA and titleB: TWO distinct title options for an A/B test — same video, genuinely different hooks/angles (e.g. one framed as a direct "why" problem+solution, the other as a number/list framing, or a curiosity-gap question), not just two rewordings of the same sentence. Each MUST present a concrete problem and promise a solution — this is the most important rule and overrides any pull toward something clever-sounding. Push the promise as big and specific as the script honestly supports — a bold claim about a concrete timeframe, a specific number, or a surprising hidden cause earns far more clicks than a mild description (e.g. "The Real Reason You Can't Focus (It's Not What You Think)", "I Fixed My Anxiety In One Week Doing This", "3 Signs You're About to Burn Out"). The bigger the promise, the more the script must actually pay it off — never promise something the video doesn't deliver, since YouTube ranks on watch time, not clicks, so an unearned promise costs more than it gains. Strictly forbidden: artistic, poetic, vague, or abstract titles (never something like "Memory Echoes" or "The Weight We Carry") — a viewer must understand exactly what problem this video solves within one glance. Prefer the pattern "[Problem statement or question] (And How to [Solution/Fix])" (e.g. "Why You Can't Let Go of the Past (And How to Stop)", "5 Signs You're Burning Out (And What Actually Helps)") — a number or a direct why/how framing both work as the opening hook, but each title must always resolve toward a solution being promised, never just name a feeling. Within that same problem+solution requirement, still push titleA and titleB toward genuinely different ANGLES on it, not just different wording of the same angle — e.g. one could frame the problem as a direct question the viewer's been asking themselves, while the other frames it as a mistake/warning ("X Signs You're Doing Y Wrong" style) — both still promise a concrete solution, just arrived at differently. Follow the hook with the rest of the title, then end with exactly " | The Mindful Path". Also make the two titles deliberately different in LENGTH, not just angle — make titleA short and punchy (aim for well under half the character budget below, e.g. just the hook itself), and let titleB run fuller/more descriptive using more of the available room. Keep everything before that suffix under ~52 characters so the full title stays under ~72 — this is a ceiling for titleB, not a target both should hit. Titles must be grammatically correct, complete English — when trimming for length, cut a word, never a required contraction ("you're", "it's", "don't", "can't"): a real title this exact prompt once produced was "Why You Stuck on Crossword Clues" (wrong — missing "'re"), which must read "Why You're Stuck on Crossword Clues" instead.
- thumbnailTextA and thumbnailTextB: STRICTLY 3-4 words each (never more — this is a hard limit, not a target), written to sit as large bold text on a thumbnail image (short, punchy, high-curiosity — think of the biggest, simplest words that would work shouted across a room). Each must pair with its matching title (A with A, B with B) but must NOT restate or shorten that title — give it a different angle or emotional beat from the same topic. No trailing punctuation. The two thumbnail texts should also read differently from each other. Grammatically correct here too — never drop a required contraction to hit the word count.
- mayaExpressionA and mayaExpressionB: for each thumbnail text, a short (2-4 word) description of the facial expression/pose Maya should have on that thumbnail so it visually matches the text's emotional beat (e.g. "shocked, wide eyes", "calm knowing smile", "pointing, determined", "stressed, hand on head"). Must pair with its matching thumbnail text's emotional tone, and the two should differ from each other the same way the two thumbnail texts do.
...
- tags: 10-15 short relevant keywords/phrases for YouTube SEO (lowercase, no # symbol)`;
  ...
  try {
    ...
    return {
      ...
      mayaExpressionA: parsed.mayaExpressionA || "",
      mayaExpressionB: parsed.mayaExpressionB || "",
