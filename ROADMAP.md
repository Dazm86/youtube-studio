...
### 2026-08-27 — Trend Finder integration fixes + empty-response bug (short scripts) + new "fully automatic" button
Three separate asks in one session, on top of the Trend Finder feature added earlier the same day. **On branch `fix/trend-finder-auto-produce` (commit `6377a9e`), pushed but not yet merged to `main`** — see PROJECT_STATE.md's "Last synced against" note.

**Trend Finder's two "unverified" integration guesses** (`TREND_FINDER_INTEGRATION.md`), checked against the real files:
1. `lib/trends/analyzer.js`'s `generateText({..., system})` call — real bug. `providers/router.js`'s `generateText()` didn't accept a `system` param at all; a plain object-destructure silently drops unknown keys, so it wasn't a crash, just silent data loss — the strict "reply with ONLY a JSON array" system instruction never actually reached the model. Reliability wasn't catastrophic (the same instruction is also baked into the end of the prompt string as a fallback) but was lower than intended. Fixed by adding real `system` support to `generateText()` and all three text adapters: `groq`/`openai` prepend it as a `role:"system"` message, `anthropic` passes it as the top-level `system` field (the correct way for the Messages API — not a message-role trick).
2. The two session-gated trends routes' `getServerSession`/`authOptions` imports — turned out cosmetic, not a real bug (`next-auth/next` does export `getServerSession` in this NextAuth v4 setup too), but inconsistent with the other ~30 routes in the app (`next-auth`, no `/next`; `@/lib/auth/authOptions`, no `.js`). Aligned both routes to match the established convention.
3. **Bonus catch** found while tracing the Trend Finder → production handoff end-to-end: despite what this file's "Key flows" section already claimed, `VideoStudio.js` never actually read the `?topic=` query param that `/trends`'s Approve button links to (`/long?topic=...` / `/short?topic=...`) — so approving a topic did **not** pre-fill the form; the claim in this doc was aspirational, not implemented. Genuinely fixed now: `VideoStudio.js` reads `useSearchParams().get("topic")` on mount into initial `topic` state; `long/page.js`/`short/page.js` wrap it in `<Suspense>` (the App Router requires this for any component calling `useSearchParams()`).

**Root-caused and fixed the "پاسخ خالی از هوش مصنوعی دریافت شد" (empty AI response) error, specifically on short-video scripts.** `openai/gpt-oss-120b` (Groq's text model) is a real reasoning model — Groq's docs confirm `reasoning_effort` defaults to **medium**, not low, whenever a request doesn't set it explicitly. A prior session's comment in `script/index.js` assumed `reasoning_effort="low"` was already being sent, and sized `maxTokens` (700 for short scripts) around that assumption — but `providers/registry.js`'s `groqText()` never actually included the parameter in the request body, so the model ran at the heavier true default. With a 700-token cap, the model could burn its *entire* budget on hidden reasoning tokens and hit `finish_reason="length"` before writing a single token of the visible answer — `content` came back completely empty. Long-form scripts (3000-token cap) essentially never hit this, hence why it was short-video-specific. Three-layer fix, so it's fixed at the root but also can't fully break generation if it recurs:
1. `groqText()` now actually sends `reasoning_effort: "low"` (guarded to only apply to `gpt-oss` models, so it's a no-op if `GROQ_TEXT_MODEL` is ever changed to a non-reasoning model).
2. Short-script `maxTokens` raised 700 → 1200 for extra margin (reasoning-token usage still has some variance even at "low").
3. Separately-found, more general bug in `providers/router.js`: an *empty* text response wasn't treated as a failure at all — `tryProviders()` returned normally since no exception was thrown, so even a properly-configured fallback provider never got a chance to run. Now an empty text result is treated exactly like any other provider error: retried on the same provider up to twice (matching the file's existing rate-limit/timeout retry pattern, since this is the common single-provider case), then falls through to the next configured provider.

**New: "تولید کاملاً خودکار" (fully automatic production) button**, one on each of `/long` and `/short`. Runs, in order: pick the best `approved`-but-not-yet-`produced` Trend Finder topic → write the script → suggest title/thumbnail/description/tags → the existing 5-stage `runPipeline()` (voice → media → render → upload → thumbnail/captions/community-post). Design choices (the user explicitly left step order/detail to be filled in):
- Topic selection is one atomic `UPDATE trend_topics ... WHERE id = (SELECT ... FOR UPDATE SKIP LOCKED)` — new `claimNextApprovedTopic()` in `trends/db.js` — so a double-click (or two people running it at once) can't both grab the same topic.
- Only ever pulls from `approved` topics — the existing human-approval gate this project owner explicitly wanted is untouched; this button only automates what happens *after* approval, it doesn't bypass it.
- On any failure after claiming, `releaseTrendTopicClaim()` puts the topic back to `approved` so it isn't stranded and can be retried; on success, `markTrendTopicProduced()` sets `status='produced'` with the real `video_id`.
- Deliberately **always runs in-process**, never through `USE_RENDER_WORKER`'s worker/jobs path — same choice already made for the scheduler and repurpose flows (Key flows #3/#4). Reasoning: the in-process path's existing heartbeat + self-ping keepalive already solves the Render free-tier sleep problem on its own (the worker is an optional offload, not a requirement for surviving long renders), and wiring this feature's topic-claim lifecycle to an *async* worker completion would have collided with the still-open worker-callback credential-expiry bug (see Known issues) in ways that felt genuinely unreliable rather than just "one more edge case."
- Reuses `VideoStudio.js`'s existing `generatingVideo`/`videoGenStatus`/`videoGenProgress` state for its progress display, and live-fills the topic/script/title/tags form fields as each is generated — so the result lands in the normal, editable form afterward instead of being a black box the user can't inspect or tweak.

Files: `lib/providers/registry.js`, `lib/providers/router.js`, `lib/script/index.js`, `lib/trends/db.js` (all edited), `app/api/trends/scan-now/route.js`, `app/api/trends/[id]/route.js`, `app/long/page.js`, `app/short/page.js`, `components/studio/VideoStudio.js` (all edited), `lib/autoProduce.js`, `app/api/auto-produce/route.js` (both new).
